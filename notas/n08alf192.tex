...
Facultad de Ciencias UNAM\footnote{Material elaborado en el marco del proyecto
  PAPIME PE102117}\\ 
Nota de Clase 8: Autómatas de Pila} 
...
si $(p,\gamma)\in \delta(q,a,\sigma)$ entonces estando en el estado $q$ y leyendo 
$a$ con $\sigma$ en el tope de la pila, el aut\'omata hace lo siguiente:
\bi
 \item consume el s\'imbolo $a$
 \item elimina $s$ del tope de la pila, es decir \verb=POP=($\sigma$)
...
\bi
 \item $(q',\sigma)\in\delta(q,a,\sigma)$\\
  en este caso el contenido de la pila no se ha alterado.
 \item $(q',\vacia)\in\delta(q,a,\sigma)$\\
  el s\'imbolo $\sigma$ se borra y el aut\'omata tiene un nuevo tope en la 
  pila, es decir, el s\'imbolo colocado inmediatamente abajo de $\sigma$.
 \item $(q',\gamma)\in\delta(q,\vacia,\sigma)$\\
  esta es una $\vacia$-transici\'on, el s\'imbolo de entrada no se 
  procesa, se cambi\'o de estado y adem\'as el tope de la pila $\sigma$ se 
  reemplaza por $\gamma$.
\ei
...
\bi
 ...
 \item $\sigma\beta$ es el contenido total de la pila, siendo $\sigma$ el        
...
\[
\pt{q,aw,\sigma\beta}\vdash_M \pt{p,w,\gamma\beta}\;\;\text{ si y s\'olo si } \;\;
(p,\gamma)\in\delta(q,a,\sigma)
\]
% \bi
...
%  \item $p$ es el nuevo estado de la m\'aquina
%  \item $w$ es la nueva cadena a procesar
%  \item $\gamma \beta$ es el contenido de la pila en donde $\gamma$ est\'a en el 
%     tope
% \ei  
Es importante remarcar que del lado derecho, en el contenido de la pila, que es
$\gamma\beta$, se entiende que el tope de la pila contiene al primer símbolo
de $\gamma$.
...
La relaci\'on $\vdash^\star$ entre configuraciones $\C,\C'$ es la
cerradura reflexiva transitiva de $\vdash$ y formaliza la 
noción de múltiples pasos de c\'omputo). se define mediante las siguientes reglas:
\begin{itemize}
\item La relación $\vdash^\star$ incluye cero pasos de reescritura
\[
\frac{}{\C\vdash^\star \C}\;(C1)
\]
\item $\vdash^\star$ consiste en dar muchos pasos (tal vez cero) y un paso de reescritura
\[
\frac{\C\vdash^\star\C'\;\;\;\C'\vdash\C''}{\C\vdash^\star\C''}\;(C2)
\] 
\item Alternativamente, $\vdash^\star$ consisten en dar primero un paso de
  reescritura, seguido de muchos pasos (tal vez cero).
\[
\frac{\C\vdash\C'\;\;\;\C'\vdash^\star\C''}{\C\vdash^\star\C''}\;(C3)
\] 
\end{itemize}
...
Dado que las formas de aceptación son equivalentes, definimos lo siguiente: 

\begin{definition}Un lenguaje $L\inc\sest$ es de tipo 2 (o libre de
  contexto\footnote{Más adelante sabremos la razón de este nombre}) si $L$ es
  aceptado por un autómata de pila.
\end{definition}
...
los pal\'indromos. 
Esto implica que hay lenguajes tipo 2 que no pueden ser 
