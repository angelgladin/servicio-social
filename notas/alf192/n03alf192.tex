...
Como hemos comentado en clase, un aut\'omata es una representaci\'on abstracta de una 
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
 \item Un estado final~$q_f$ se representa por: \hspace*{30pt}
\begin{center}  
\begin{tikzpicture}[node distance=2cm,every node/.style={scale=0.8},semithick]
...
\subsection{Aut\'omatas y lenguajes}
...
soluci\'on. Por ejemplo, el autómata discutido anteriormente, reconoce al
lenguaje $a^*+a^*ba^*ba^*$ es decir, el lenguaje regular
$L=\{w\in\{a,b\}^\star\mid
w\mbox{ tiene u número par de bes }\}$ 

El primer paso para resolver los problemas anteriores es la formalizaci\'on de la noci\'on de 
...
\bi
 \item $\dest(q,\vacia) = q$
 \item $\dest(q,wa) = \delta\big(\dest(q,w),a\big)$, o alternativamente
     $\dest(q,aw) = \dest\big(\delta(q,a),w\big)$.
\ei
}
...
\be
 ...
 \item La palabra contiene $00$ pero no $11$:
  \bi
   \item se ley\'o otro $0$  
   \item se ley\'o un $1$ \hspace*{30pt}
  %  \begin{tikzpicture}[node distance=2.5cm,every 
  %   node/.style={scale=0.8},semithick]
  %   \node[state] (qi) {$q_i$};
  %   \node[state] (qj) [right of=qi] {$q_j$};
  ...
  % \end{tikzpicture}
  \ei
 \item La palabra contiene la subcadena $11$. \hspace*{30pt}
   \begin{itemize}
   \item Se leyó otro $1$.
   \end{itemize}
Este análisis da un conjunto de seis estados, tres en el primer grupo, dos en
...
\todo{[START] Duda}
TODO: PONER AQUÍ EL AUTOMATA DEL EJEMPLO DE ARRIBA
\todo{[END] Duda}
...
\be
 \item Primero se dise\~na la parte de $M^c$ que acepta la cadena $abaab$:
 \begin{center}
  \begin{tikzpicture}[node distance=2.5cm,every 
    node/.style={scale=0.8},semithick]
    \node[state,initial,initial text=] (q0) {$q_0$};
  ...
  \end{tikzpicture}
 \end{center}
 \item Se completa el aut\'omata anterior y se obtienen los complementos de 
  los estados para obtener $M$:
  \begin{center}
  \begin{tikzpicture}[node distance=2.5cm,every 
    node/.style={scale=0.8},semithick]
    \node[state,initial,initial text=,accepting by double] (q0) {$q_0$};
    ...
    \path[->] (q0) edge node [above] {a} (q1);
    \path[->] (q1) edge [bend left] node [above] {b} (q2);
    ...
    \path[->] (q2) edge [bend left] node [above] {a} (q1);
    \path[->] (q3) edge [bend left] node [below] {b} (q2);
    \path[->] (q4) edge [bend left] node [below] {a} (q1);
  \end{tikzpicture}
 \end{center}
\ee
...
\todo{[START] Ejemplo}
\paragraph{Ejemplo:}
Sea $\Sigma = \{ a, *, / \}$, diseñar un AFN para
$$ L = \{ w \mid w\,\mbox{no es un comentario en el leguaje de programación C} \} $$
...
\be
 \item Primero diseñar $\overline{L} = \{ w \mid w\,\mbox{es un comentario en el leguaje de programación C} \}$:
 \begin{center}
  \begin{tikzpicture}
    ...
    \draw
        ...
        (q2)
            ...
            edge [bend left] node {$*$} (q3)
        (q3)
            edge [loop below] node {$*$} ( )
            edge [bend left] node {$a$} (q2)
            edge node {$/$} (q4)
        (q4)
            edge [bend right] node [above] {$\Sigma$} (q5)
        (q5)
            edge [loop above] node {$\Sigma$} ( );
  \end{tikzpicture}
 \end{center}
 
 \item Se cambian los estados finales a $Q - F$ para obtener $M$:
  \begin{center}
    \begin{tikzpicture}
      \node[state, initial, accepting] (q0) {$q_0$};
      \node[state, accepting] (q1) [right of=q0] {$q_1$};
      \node[state, accepting] (q2) [right of=q1] {$q_2$};
      \node[state, accepting] [above of=q2] (q5) {$q_5$};
...
\todo{[START] Ejemplo}
\paragraph{Ejemplo:}
Sea $\Sigma = \{ 0, 1 \}$, diseñar un AFN para

$$ L = \{ w\in\{0,1\}^\star\mid w\,\mbox{no contiene }00 \} $$

\be
 \item Primero se diseña la parte de $M^c$ que acepta la cadena $00$:
 \begin{center}
  \begin{tikzpicture}
    \node[state, initial] (q0) {$q_0$};
    \node[state] (q1) [right of=q0] {$q_1$};
 ...
 \item Se obtienen los complementos de los estados para obtener $M$:
 \begin{center}
  \begin{tikzpicture}
    \node[state, initial, accepting] (q0) {$q_0$};
    \node[state, accepting] (q1) [right of=q0] {$q_1$};
...
\todo{[START] Ejemplo}
\paragraph{Ejemplo:}
Consideremos los lenguajes $L_1$ y $L_2$ tales que
$$ L_1=\{w\in\{0,1\}^\star\mid w \mbox{ tiene un número par de }1\mbox{'s}\} $$
$$ L_2= \{w\in\{0,1\}^\star\mid w \mbox{ tiene un número impar de }0\mbox{'s}\} $$

A continuación se construyen los autómatas para reconocer los respectivos lenguajes y después se obtiene $M$
usando la definición anterior, se proporciona la tabla de transiciones.
...
\begin{center}
  ...
  \begin{tikzpicture}
    ...
    \draw
        (p0)
            edge [loop above] node {1} ( )
            edge [bend left] node {0} (p1)
        (p1)
            edge [loop above] node {1} ( )
            edge [bend left] node {0} (p0);
    \end{tikzpicture} 
  \end{tabular}
\end{center}
...
\begin{array}[c]{|c|c|c|c|}\hline 
  \delta_1 \times \delta_2  &  0  & 1 & \text{ renombre }\\\hline 
  (q_0,p_0) & (q_0,p_1) & (q_1,p_0) & r_0 \\\hline 
  (q_0,p_1) & (q_0,p_0) & (q_1,p_1) & r_1 \\\hline 
  (q_1,p_0) & (q_1,p_1) & (q_0,p_0) & r_2 \\\hline 
  (q_1,p_1) & (q_1,p_0) & (q_0,p_1) & r_3 \\\hline
\end{array}
\]

\be
  \item Autómata $M$ que reconoce el lenguaje $L_1 \cup L_2$, con $F = \{ r_0, r_1, r_3 \}$:
  \todo{[START] Ejemplo, unión}
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw
          (r0)
              ...
              edge [bend left] node {1} (r2)
          (r1)
              edge [bend left] node {0} (r0)
              edge [bend left] node {1} (r3)
          (r2)
              edge [bend left] node {1} (r0)
              edge [bend left] node {0} (r3)
          (r3)
  ...
  \item Autómata $M$ que  reconoce el lenguaje $L_1 \cap L_2$, con $F = \{ r_1 \}$:
  \todo{[START] Ejemplo, intersección}
  \begin{center}
    \begin{tikzpicture}
  ...
  \item Autómata $M$ que  reconoce el lenguaje $L_1 - L_2$, con $F = \{ r_0 \}$:
  \todo{[START] Ejemplo, diferencia}
...
\subsection{Ejemplos de Autómatas Finitos Deterministas}
\todo{[START] Ejemplos}
\paragraph{Ejemplo:}
$$ L = \{ w \in \{ 0,1 \}^\star \mid w\,\mbox{contiene la subcadena }1001 \} $$
...
\paragraph{Ejemplo:}
$$ L = \{ w \in \{ 0,1 \}^\star \mid w\,\mbox{termina con }111 \} $$
  ...
  \begin{tikzpicture}
    ...
    \draw
        ...
        (q1)
            ...
            edge node {1} (q2)
        (q2)
            edge [bend left=45] node {0} (q0)
            edge node {1} (q3)
        (q3)
            edge [loop above] node {1} ( )
            edge [bend left=60] node {0} (q0);
  \end{tikzpicture}
}

\paragraph{Ejemplo:}
$$ L = \{ w \in \{ 1,2,3 \}^\star \mid w\,\mbox{contiene la subcadena }123 \} $$
...
\TransitionTableAndAutomataSideBySide {
  ...
  \begin{tikzpicture}
    ...
    \draw
        (a)
            edge [bend right] node {1} (b)
            edge[loop above] node {2,3} (a)
        (b) edge [bend right] node[above] {3} (a)
            edge [bend right] node {2} (c)
            edge[loop above] node {1} (b)
        (c) edge node {3} (d)
            edge [bend right] node[above] {1} (b)
            edge [bend left=45] node[above] {2} (a)
        (d)
            edge[loop above] node {1,2,3} (d);
  \end{tikzpicture}
}
\todo{[END] Ejemplos}
