...
\begin{enumerate}
  ...
  \item Otra cadena de dígitos. Tanto puede ser ésa cadena, o la cadena (2) puede ser vacía, pero al menor una de las
...
\begin{itemize}
  ...
  \begin{enumerate}
    ...
    $Cl_{\vacia}(q_1) \cup Cl_{\vacia}(q_4) = \{q_1\} \cup \{q_4\} = \{q_1, q_4\}$. Que es $\dest(q_0, \texttt{5})$.
    Este patrón de dos pasos se repite para los siguientes dos símbolos.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Después, $\dest(q_0, \text{5.6}) = Cl_{\vacia}(q_3) = \{q_3, q_5\}$.
  \end{enumerate}

\end{itemize}

Teniendo así que $\dest(q_0, \texttt{5.6})$ contiene el estado de aceptación $q_5$, así la cadena \texttt{5.6} está en
el lenguaje de ese AFN$\vacia$.
