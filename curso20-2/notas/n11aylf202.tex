...
Los lenguajes regulares, como lenguajes sencillos de reconocer por una 
m\'aquina, encuentran su mancuerna con los aut\'omatas finitos los cuales son 
...
finitos, que puede realizarse de diversos modos:
\bi
...
caracter\'isticas m\'as complejas de un lenguaje al tener un dispositivo m\'as 
...
es decir una estructura de datos para almacenar objetos. 
...
\begin{wrapfigure}{r}{0.45\textwidth}
 \begin{center}
  \vspace*{-30pt}
  \includegraphics[scale=0.20]{pila}
 \end{center}
\end{wrapfigure}
...
Estas funciones s\'olo acceden al \textbf{tope} de la pila que es exactamente el 
\'ultimo elemento en ser almacenado. Si no hay elementos almacenados decimos 
que la pila est\'a vac\'ia.\\
Esta caracter\'istica le da otro nombre a las pilas, se llaman estructuras 
\textbf{\enquote{Last In First Out} (LIFO)} es decir que el \'ultimo elemento en 
ser almacenado ser\'a el primero en salir de la pila.\\ 
...
aut\'omata 
$$ M_3 = \pt{\{q_0,q_1,q_2\},\{a,b,c\},\{A,B,Z_0\},\delta,q_0,Z_0,\{q_2\}}$$
con un \'unico estado final $q_2$ y cuya representaci\'on gr\'afica es la 
siguiente:
\begin{center}
\begin{tikzpicture}[node distance=4cm,every node/.style={scale=0.85},semithick]
    \node[state,initial,initial text=] (q0) {$q_{0}$};
    \node[state] (q1) [right of q0,xshift=9cm] {$q_1$};
    ...
    \path[->] (q0) edge node [above] {$\qquad\qquad c,\,A/A\quad 
c,\,Z_0/Z_0$} node [below] {$c,\,B/B$} (q1);
...
\noindent La prueba consiste en construir un nuevo aut\'omata que acepta el 
mismo lenguaje con la pila vac\'ia, la idea es construir un aut\'omata que 
vaci\'ie la pila una vez que se ha llegado a un estado final. \\
...
\bi
 ...
 \item la funci\'on de transici\'on conserva todas las transiciones de~$M$ 
  ...
  \bi
   \item[] $\delta(p_0,\vacia,N_0)=\{(q_0,Z_0N_0)\}$  (inicializaci\'on)
   \item[] $(p,s)\in\delta(q_f,\vacia,s)$, para todo 
   $q_f\in F,\;s\in\Gamma\cup\{N_0\}$    (transici\'on de final al nuevo estado 
$p$)
   \item[] $\delta(p,\vacia,s)=\{(p,\vacia)\}$ ($p$ es el estado para vaciar la 
pila)
  \ei
\ei      
Como ejemplo de esta construcci\'on, a continuaci\'on est\'a el aut\'omata que 
acepta por pila vac\'ia a partir de $M_3$ dado para $L_3= \{wcw^R \mid 
w\in\{a,b\}^\star\}$:
$$ M'_3 = 
\pt{\{p_0,p,q_0,q_1\},\{a,b,c\},\{A,B,Z_0,N_0\},\delta,p_0,N_0,\{q_2\}}$$
\[
 \begin{array}{rclcrcl}
 \delta(p_0,\_,N_0) & = & \{(q_0,Z_0N_0)\} & \qquad 
  \_\in\{a,b\} &&&\\
 ...
 \delta(q_1,b,B) & = & \{(q_1,\vacia)\} & \qquad \qquad 
  ...
  \delta(q_2,\vacia,Z_0) & = & \{(p,Z_0) \} &\qquad
  & \delta(p,\vacia,Z_0)& = & \{(p,\vacia)\}
 \end{array}
\]
...
\bi
 ...
 \item la funci\'on de transici\'on conserva todas las transiciones de~$M$ y 
  ...
  \bi
   ...
   \item[] $(p_f,N_0)\in\delta(q,\vacia,N_0)$, para todo $q\in Q$
  \ei  
\ei
Como ejemplo para esta construcci\'on, veamos el caso de $L_2 =\{a^ib^j \mid 
i\geq j\geq 0\}$.
El siguiente es el aut\'omata $M'_2$ que acepta el mismo lenguaje por estado 
final:
$$  M_2=\pt{\{p_0,p_f,q_0,q_1\},\{a,b\},\{X,Z_0,N_0\},\delta,p_0,N_0,\{q_f\}}$$
\[
 \begin{array}{rclcrcl}
  \delta(p_0,\_,N_0) & = & \{(q_0,Z_0N_0)\} \qquad 
  \_\in\{a,b\} &&&\\
  ...
  \delta(q_1,b,X) & = & \{(q_1,\vacia)\} \qquad \qquad 
   & \delta(q_1,\vacia,Z_0) & = & \{(q_1,\vacia)\} \\
  \delta(q_1,\vacia,X) & = & \{(q_1,\vacia)\} \qquad \qquad 
   & \delta(q_1,\vacia,N_0) & = & \{q_f,N_0 \} 
 \end{array}
\]
...
pila \textbf{no} hay equivalencia entre los aut\'omatas deterministas y los 
no-deterministas.
...
los pal\'indromos, cuyo diagrama se muestra a continuaci\'on:
\begin{center}
\begin{tikzpicture}[node distance=4cm,every node/.style={scale=0.9},semithick]
    \node[state,initial,initial text=] (q0) {$q_{0}$};
    \node[state] (q1) [right of q0,xshift=11cm] {$q_1$};
    \node[state,accepting by double] (q2) [right of=q1] {$q_2$};
    \path[->] (q0) edge [in=160,out=130,loop] node [left] {$a,\,Z_0/AZ_0$} 
    (q0);
    \path[->] (q0) edge [in=110,out=80,loop] node [above] {$b,\,Z_0/BZ_0$} (q0);
    ...
    \path[->] (q0) edge [in=260,out=290,loop] node [below] {$b,\,A/BA$} (q0);
    \path[->] (q0) edge [in=310,out=340,loop] node [right] {$b,\,B/BB$} (q0);
    \path[->] (q0) edge [bend left] node [above] {$\qquad b,\,Z_0/Z_0 \quad 
b,\,A/A\quad b,\,B/B$} node [below] {} (q1);
    \path[->] (q0) edge [bend right] node[below] {$\qquad a,\,Z_0/Z_0 \quad 
a,\,A/A\quad a,\,B/B $}  (q1); 
    \path[->] (q0) edge node [above] {$\qquad\qquad \vacia,\,Z_0/Z_0\quad 
\vacia,\,A/A\quad \vacia,\,B/B$}  (q1);
    \path[->] (q1) edge [loop above] node [above] {$a,\,A/\vacia$} (q1);
    \path[->] (q1) edge [loop below] node [below] {$b,\,B/\vacia$} (q1);
    \path[->] (q1) edge node [above] {$\vacia,\, Z_0/Z_0$} (q2);
 \end{tikzpicture}
