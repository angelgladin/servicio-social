...
la computación, hasta las ingenierías dedicadas a aplicaciones espec\'ificas.
...
Mediante la teoría del cómputo, espec\'ificamente mediante la teoría de 
autómatas 
...
\bi
 ...
 \item El alfabeto ASCII: @zA\$ 
\ei
  
Obsérvese que los símbolos son a su vez cadenas que constan de un solo
caracter. Más aún podemos referirnos a la \textbf{\textit{cadena vacía}} 
(i.e. la sucesión vacía de símbolos) que se denotará con el metasímbolo 
...
\defin{
  ...
  de $\Sigma$. 
  Es decir $\Sigma^\star=\{a_1\!\ldots a_n \mid a_i\in\Sigma \land 
i\in\mathbb{N}\}$ donde 
...
\bi
 \item Decimos que $v$ es una subcadena de $u$ si existen cadenas 
  ...
  \item Decimos que $v$ es una subcadena \textit{propia} de $u$ si es subcadena 
de $u$ y adem\'as  $v\neq\vacia$, $v\neq u$,
%   $x,y\in\Sigma^\star$ tales que $u=xvy$.
...
Opcionalmente se puede dar una definci\'on equivalente 
en donde los s\'imbolos se agregen al inicio de una cadena: si $w\in\sest$ y 
$a\in\Sigma$ entonces $aw\in\sest$. En la literatura, es m\'as usada la 
convenci\'on que aparece en nuestra definici\'on al agregar un s\'imbolo al 
final.

Estas definiciones equivalentes deben usarse con cuidado y no se deben mezclar 
al momento de demostrar propiedades.
Se debe establecer claramente cu\'al es la definici\'on que se utiliza y 
respetarla para definiciones, convenciones y demostraciones. \newline

La definición recursiva anterior genera el siguiente principio de inducción
estructural: 
\bi
...
\ei
Las definiciones equivalentes usando la definici\'on de cadena al agregar 
s\'imbolos al inicio son las siguientes:
\bi
 \item La concatenación de cadenas es una función
  $\cdot:\sest\times\sest\imp\sest$ tal que:
  $$\cv\cdot u = u \qquad \qquad (av)\cdot u = a (v\cdot u)$$

 \item La longitud $|\cdot|:\sest\imp\N$ se define como:
  $$ |\vacia|=0  \qquad \qquad |av|= 1 + |v|$$

 \item La reversa de una cadena $\cdot^R:\sest\imp\sest$ est\'a definida por:
  $$ \vacia^R=\vacia \qquad \qquad (av)^R=(v^R)a $$
\ei
...
llamado el \textit{lenguaje total}. Además los conjuntos $\Sigma$ y $\{\cv\}$ 
también son lenguajes.


\subsection{Operaciones con Lenguajes}
Dado que los lenguajes son conjuntos, entonces todas las operaciones
...
Esta operaci\'on se comporta de manera especial como se observa en la siguiente 
propiedad:\\ considere dos lenguajes $L_1$ y $L_2$ entonces $(L_1 L_2)^\star 
\neq L_1^\star L_2^\star$ m\'as bien esto debe ser igual a $(L_1^\star 
L_2^\star)^\star$.
