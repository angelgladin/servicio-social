...
se ha leído $q_i$ (donde $q_i$ es un número en decimal). Si se lee 0, el nuevo número a leer
...
($2 * q_i$ ó $2 * q_i + 1$) donde $0 \leq q_i < k$. \pause

\espc
El estado inicial será $q_0$ y para verficar que si el número ees divisible su estado final (y el
único) debe ser $q_0$ porque significa que el número ya procesado su residuo fue 0.
\end{frame}
...
\begin{itemize}
  ...
  \item $\Sigma = \{ 0, 1 \}$
  \item $\delta (q_i, 0) = q_{2i \mod k}$ y $\delta (q_i, 0) = q_{(2i + 1) \mod k}$
  \item $F = \{ q_0 \}$
\end{itemize}\pause
