...
\title[AyLF MCIC 2019-1] % (optional, use only with long paper titles)
...
se denotará con $\boldsymbol{\varepsilon}$.} (otra notación común es $\Lambda$)
...
\begin{frame}
  ...
  \bi
  \item Asociatividad: $(uv)w = u(vw)$.\\ \medskip
    \item Identidad: $v\varepsilon = \varepsilon v= v$.\\ \medskip 
...
\bi
\item[] ?` Es conmutativa la concatenación ?  \pause\medskip
\item[]
?` $vw = wv$ ?  \medskip\pause
\item[] NO $jarrito\neq rritoja$
\ei
...
\item Decimos que $v$ es una subcadena de $u$ si existen cadenas
  $x,y\in\Sigma^\star$ tales que $u=xvy$.\medskip
...
Se cumplen las siguientes propiedades:
\bi
\item $L\varnothing=\varnothing L=\varnothing$\medskip
  \item $L\{\varepsilon\}=\{\varepsilon\}L=L$\medskip
    \item $L(MN)=(LM)N$\medskip
...
\begin{frame}
  ...
  L^R=\{u^R\;|\;u\in L\}
  \eeqs
Se cumplen las siguientes propiedades:\medskip
\bi
\item $(L^R)^R=L$\medskip
  \item $(LM)^R=M^RL^R$\medskip
    \item $(L\cup M)^R=L^R\cup M^R$\medskip
...
\begin{frame}
  ...
  L^i
  \eeqs
  donde $L^i=LL\ldots L$, $i$ veces ($L^0=_{def} \{\varepsilon\}$) 
\end{frame}
...
\begin{frame}
  ...
  Se cumplen las siguientes propiedades
  \bi
  \item $L^\star=L^+\cup\{\varepsilon\}$\medskip
    \item $L^\star=L^+$ si y sólo si $\varepsilon\in L$.\medskip
      \item $(L^\star)^\star=L^\star$\medskip
        \item $L^\star L^\star=L^\star$\medskip
          \item $(L^+)^\star=(L^\star)^+=L^\star$\medskip
            \item $(L^+)^+=L^+$\medskip
              \item $L^+L^+\inc L^+$
  \ei
